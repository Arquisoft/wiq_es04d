...
Las pruebas seguiran el siguiente procedimiento sencillo, pero que servira para probar los servicios y como se comportan ante el estres generado por muchos usuarios:
...
Esto se debe principalmente a las bajas prestaciones de la maquina virtual, marcadas por el credito disponible como estudiantes.
...
- El diseño de la página es bastante intuitivo para los usuarios en especial para los que tiene alto conocimiento. Los usuarios con bajo conocimiento necesitaron de una pequeña intervencion por parte del observador.
- Para los usuarios de nivel bajo se les hace dificil tener que registrarse y a continuacion tener que logearse en la página.
...
- Agregar una página de ayuda para los usuarios que no sepan que pasos seguir para jugar. Ya sea tener que registrase en la página, como juagro o como usar la API (aunque la API tenga su documentacion).
